...
describe('ItemsService', () => {
  ...
  const repo = {
    ...
    manager: {
      transaction: jest
        .fn()
        .mockImplementation((cb: (arg: typeof em) => unknown) => cb(em)),
      createQueryBuilder: jest.fn(),
    },
  };
  ...
  beforeEach(async () => {
    ...
    repo.manager.transaction.mockImplementation(
      (cb: (arg: typeof em) => unknown) => cb(em),
    );
  ...
  describe('assignToCategory', () => {
    ...
    it('skips products not found in the database', async () => {
      ...
      expect(result.skipped).toEqual([
        { id: 'missing', reason: 'product not found' },
      ]);
    });
    ...
    it('skips products already assigned to the target category', async () => {
      ...
      expect(result.assigned).toBe(1);
      expect(result.skipped).toEqual([
        { id: 'a', reason: 'already assigned to this category' },
      ]);
    });
    ...
    it('serialises the category template into the UPDATE call', async () => {
      ...
      expect(emQuery).toHaveBeenCalledWith(expect.any(String), [
        1,
    ...
    it('unassigns products and returns the count', async () => {
      ...
      expect(emQuery).toHaveBeenCalledWith(
        expect.stringContaining(
          'UPDATE exact_items SET category_id = NULL, pim_template = NULL',
        ),
        [['a', 'b']],
      );
    });
  ...
  describe('create', () => {
    ...
    it('fetches the category and copies its template when categoryId is provided', async () => {
      ...
      await service.create(
        { description: 'Widget', categoryId: 1 },
        'admin@test.com',
      );
    ...
    it('throws NotFoundException when categoryId does not exist', async () => {
      ...
      await expect(
        service.create({ description: 'Widget', categoryId: 99 }),
  ...
  describe('updatePimTemplate', () => {
    ...
    it('accepts null pimTemplate to clear the template', async () => {
      ...
      await service.updatePimTemplate('item-uuid-1', {
        pimTemplate: null,
      });
